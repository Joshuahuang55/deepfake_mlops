...
import time  # MLops新增
import mlflow # MLops新增
from src.mlops_config import MLOpsConfig  # MLops新增
...
def main():
    ...
    # --- MLOps Init ---
    mlops_active = MLOpsConfig.setup_mlflow()
    if not mlops_active:
        st.toast("⚠️ MLOps Server Disconnected", icon="🔌")
    ...
    if uploaded_file is not None:
        ...
        with col_main:
            with st.spinner("🔬 Running forensic analysis algorithms..."):
                ...
                # ==========================================
                # [NEW] MLOps: Logging & Active Learning
                # ==========================================
                if mlops_active:
                    try:
                        with mlflow.start_run(run_name=f"Inference_{int(time.time())}"):
                            ...
                            # 2. 困難樣本篩選 (Hard Sample Mining)
                            # 設定閾值：例如信心分數 < 85% 視為模型「不確定」
                            CONFIDENCE_THRESHOLD = 0.85 
                            
                            if score < CONFIDENCE_THRESHOLD:
                                # 顯示通知給使用者 (可選)
                                st.toast(f"Low confidence ({score:.1%}). Flagging for retraining.", icon="🚩")
                                
                                # 上傳圖片到 MinIO
                                success, s3_path = MLOpsConfig.log_hard_sample(
                                    image, 
                                    uploaded_file.name, 
                                    score, 
                                    label
                                ...
                                if success:
                                    # 在 MLflow 標記這筆資料有上傳圖片
                                    mlflow.log_param("hard_sample_path", s3_path)
                                    mlflow.set_tag("data_type", "hard_sample")
                                    print(f"✅ Hard sample saved: {s3_path}")
                                else:
                                    print(f"❌ Failed to save sample: {s3_path}")
                            else:
                                mlflow.set_tag("data_type", "normal")
        ...
        # ==========================================
        # [NEW] MLOps Step 2: Human Feedback Loop
        # ==========================================
        st.markdown("---")
        st.markdown("### 🗣️ Human-in-the-Loop Feedback")
        
        st.info(
            """
            **Help refine the system!** If the model is confident but wrong (False Positive/Negative), please flag it below. 
            Your feedback collects "Hard Samples" to retrain the model and fix its blind spots.
            """
        )
        ...
        # 定義回饋邏輯
        with col_f1:
            # 如果模型說是 REAL，但你覺得是 FAKE
            if st.button("🚨 Report Error: Actually FAKE"):
                if mlops_active:
                    with st.spinner("Logging error case to MLOps Storage..."):
                        # 強制上傳，不管信心分數多少
                        success, s3_key = MLOpsConfig.log_hard_sample(
                            image, 
                            f"CORRECTION_FAKE_{uploaded_file.name}", # 檔名加註 CORRECTION
                            score, 
                            "FAKE" # 強制標記為正確的 Ground Truth
                        )
                        
                        if success:
                            # 翻譯後的成功訊息
                            st.success("✅ Feedback Logged! Image saved to 'Hard Sample Store' for next retraining cycle.")
                            
                            # 記錄到 MLflow
                            with mlflow.start_run(run_name=f"Feedback_{int(time.time())}", nested=True):
                                mlflow.log_metric("confidence", score)
                                mlflow.log_param("user_feedback", "Misclassification")
                                mlflow.log_param("actual_label", "FAKE")
                                mlflow.log_param("predicted_label", label)
                                mlflow.log_param("s3_path", s3_key)
                        else:
                            st.error("❌ Upload failed. Please check MLOps connection.")

        with col_f2:
            if st.button("🚨 Report Error: Actually REAL"):
                if mlops_active:
                    with st.spinner("Logging error case to MLOps Storage..."):
                        success, s3_key = MLOpsConfig.log_hard_sample(
                            image, 
                            f"CORRECTION_REAL_{uploaded_file.name}", 
                            score, 
                            "REAL" # 強制標記為正確的 Ground Truth
                        )
                        ...
                        if success:
                            ...
                            with mlflow.start_run(run_name=f"Feedback_{int(time.time())}", nested=True):
                                mlflow.log_metric("confidence", score)
                                mlflow.log_param("user_feedback", "Misclassification")
                                mlflow.log_param("actual_label", "REAL")
                                mlflow.log_param("predicted_label", label)
                                mlflow.log_param("s3_path", s3_key)
                        else:
                            st.error("❌ Upload failed. Please check MLOps connection.")
